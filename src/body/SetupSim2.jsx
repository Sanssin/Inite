...
const SetupSim2 = () => {
  ...
  const isFormValid = isActivityValid && isThicknessValid && isDistanceValid; 

  // Efek pertama untuk inisialisasi data induk
  useEffect(() => { 
    loadBackendData(); 
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Efek kedua yang memantau perubahan tipe sumber radiasi
  useEffect(() => {
    if (sourceType && backendStatus !== 'loading') {
      loadMaterialData(sourceType);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const loadBackendData = async () => {
    try {
      ...
      if (isotopeResult && isotopeResult.success) {
        setIsotopeDetails(isotopeResult.data);
        setAvailableIsotopes(Object.keys(isotopeResult.data));
        setBackendStatus('connected');
        // Langsung kirim tipe default ke fungsi pemuat material agar sinkron
        await loadMaterialData('cs-137');
      } else {
        // Jika response didapat tapi struktur data bermasalah / gagal
        setIsotopeDetails(isotopeResult?.data || backendDataService.getFallbackIsotopeData());
        setBackendStatus('fallback');
        handleFallbackData('cs-137');
      }
    } catch (error) {
      console.error('Failed to load backend data, activating local fallback:', error);
      setBackendStatus('fallback');
  ...
  const loadMaterialData = async (currentSource) => {
    try {
      const sourceKey = currentSource || sourceType;
      const materialResult = await backendDataService.getMaterialDetails(sourceKey);
      ...
      if (materialResult && materialResult.success) {
        ...
        // Fallback jika respons sukses bernilai false
        handleFallbackData(sourceKey);
      }
    } catch (error) {
      console.error('Failed to load material data, activating local fallback:', error);
      handleFallbackData(currentSource || sourceType);
    }
  };

  // Fungsi pembantu khusus untuk memasok data lokal jika koneksi API terputus
  const handleFallbackData = (sourceKey) => {
    const fallbackData = backendDataService.getFallbackMaterialData(sourceKey);
    setMaterialDetails(fallbackData);
    if (fallbackData) {
      setAvailableMaterials(Object.keys(fallbackData));
    } else {
      // Hard fallback jika file service data gagal mengembalikan objek
      setAvailableMaterials(['lead', 'concrete', 'glass', 'steel']);
    }
  };
              ...
              <Form className="mt-4" style={{ textAlign: 'left' }}>
                ...
                <Form.Group className="mb-4 d-flex flex-column align-items-center">
                  ...
                  <Form.Control
                    ...
                    onChange={(e) => setShieldingThicknessInput(e.target.value)} // Tetap diarahkan ke setter ketebalan Anda jika diperlukan
